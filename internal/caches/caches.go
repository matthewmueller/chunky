...
func Download(ctx context.Context, repo repos.Repo, url *url.URL) (*Cache, error) {
	cache, err := Load(url)
	if err != nil {
		return nil, err
	}
...
func Load(url *url.URL) (*Cache, error) {
...
func (c *Cache) Download(ctx context.Context, repo repos.Repo) error {
...
func (c *Cache) Get(fileId string) (file *commits.File, ok bool) {
	file, ok = c.files[fileId]
	return file, ok
}

func (c *Cache) Set(commitId string, commit *commits.Commit) error {
	// Skip if we already have the commit
	if _, ok := c.commits[commitId]; ok {
		return nil
	}
